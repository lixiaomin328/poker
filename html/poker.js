...
let expName = 'poker';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  hellow_word = new visual.TextStim({
    win: psychoJS.window,
    name: 'hellow_word',
    text: 'hellow word\n\n',
    font: 'Arial',
...
function trialRoutineBegin() {
  ...
  trialComponents.push(hellow_word);
...
function trialRoutineEachFrame() {
  ...
  // *hellow_word* updates
  if (t >= 0.0 && hellow_word.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
    hellow_word.tStart = t;  // (not accounting for frame time here)
    hellow_word.frameNStart = frameN;  // exact frame index
    hellow_word.setAutoDraw(true);
  }

  frameRemains = 0.0 + 5.0 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
  if (hellow_word.status === PsychoJS.Status.STARTED && t >= frameRemains) {
    hellow_word.setAutoDraw(false);
  }
